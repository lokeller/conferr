...
<xsl:stylesheet version="1.0" 
 ...
 myRandNext mySingleRandDistFun randScale x"
>
  ...
  <xsl:template name="scaleSequence">
    ...
    <xsl:variable name="vrtfCurriedScale">
      <xsl:call-template name="curry">
        ...
        <xsl:with-param name="arg3" select="$pEnd"/>
      </xsl:call-template>
    </xsl:variable>
  ...
  <xsl:template name="randomSequence">
    ...
    <xsl:choose>
      <xsl:when test="$pStart = 0 and $pEnd = $modulus - 1">
        <xsl:copy-of select="vendor:node-set($vrtfUnscaledSeq)/*"/>
      </xsl:when>
      <xsl:otherwise>
		    <xsl:call-template name="scaleSequence">
			    <xsl:with-param name="pStart" select="$pStart"/>
			    <xsl:with-param name="pEnd" select="$pEnd"/>
			    <xsl:with-param name="pList" select="vendor:node-set($vrtfUnscaledSeq)/*"/>
		    </xsl:call-template>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
...
<!--
  ...
  <xsl:template name="dist-randomSequence">
    ...
    <xsl:call-template name="map">
      <xsl:with-param name="pFun" select="vendor:node-set($vrtfDistFunction)/*"/>
  ...
  <xsl:template name="randomSubSequence">
    ...
    <xsl:call-template name="randomSequence">
        ...
        <xsl:with-param name="pSeed" 
                        select="vendor:node-set($vrtfInitSequence)/*[last()]"/>
        <xsl:with-param name="pStart" select="$pStart"/>
        <xsl:with-param name="pEnd" select="$pEnd"/>
    </xsl:call-template>
  </xsl:template>
...
<!--
  ...
  > randScale s t n = ceiling (rndScl ((t - s + 1)/dintRange ) s n)
...
-->  
  <xsl:template name="_randScale" match="randScale:*">
